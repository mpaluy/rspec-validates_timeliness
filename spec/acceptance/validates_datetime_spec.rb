...
class ValidatesDatetime
  ...
  validates_datetime :started_at, is_at: -> { DateTime.current }
  validates_datetime :finished_at, after: -> { 1.week.ago }, before: -> { 1.week.since }
  validates_datetime :created_at, on_or_after: -> { 1.year.ago }, on_or_before: -> { 1.year.since }
