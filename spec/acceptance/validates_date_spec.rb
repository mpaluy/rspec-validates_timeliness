...
class ValidatesDate
  ...
  validates_date :date_of_birth, is_at: -> { Date.current }
  validates_date :date_of_start, after: -> { 1.week.ago }, before: -> { 1.week.since }
  validates_date :date_of_finish, on_or_after: -> { 1.year.ago }, on_or_before: -> { 1.year.since }
